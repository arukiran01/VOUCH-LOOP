...
const renderBrandLogo = (brandName: string) => {
  ...
  if (normalized.includes('amazon')) {
    return (
      <div className="w-10 h-10 rounded-xl bg-zinc-950 border border-amber-500/30 flex flex-col items-center justify-center font-black select-none flex-shrink-0 relative overflow-hidden">
        ...
        <span className="text-[11px] text-[#FF9900] leading-none -mt-0.5 font-bold font-sans">↵</span>
      </div>
    );
  }
  ...
  if (normalized.includes('flipkart')) {
    return (
      <div className="w-10 h-10 rounded-xl bg-[#2874f0] border border-blue-400/20 flex flex-col items-center justify-center font-extrabold text-white select-none flex-shrink-0 relative">
        <span className="text-[9px] tracking-tight leading-none uppercase font-sans">Flipkart</span>
  ...
  if (normalized.includes('ajio')) {
    return (
      <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#1b1c1e] to-[#402026] border border-rose-905/30 flex items-center justify-center font-black text-rose-100 select-none flex-shrink-0">
  ...
  if (normalized.includes('zomato')) {
    return (
      <div className="w-10 h-10 rounded-xl bg-[#e23744] border border-red-500/20 flex items-center justify-center font-black text-white select-none flex-shrink-0">
        <span className="text-xs tracking-tighter lowercase font-bold font-sans">zomato</span>
      </div>
    );
  }
  ...
  if (normalized.includes('myntra')) {
    return (
      <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-rose-500 via-purple-500 to-amber-400 border border-pink-400/25 flex items-center justify-center font-black text-white select-none flex-shrink-0 shadow-xs">
  ...
  if (normalized.includes('fastrack')) {
    return (
      <div className="w-10 h-10 rounded-xl bg-zinc-900 border border-zinc-700 flex flex-col items-center justify-center font-black text-yellow-500 select-none flex-shrink-0">
...
const getBrandTheme = (brandName: string) => {
  ...
  if (norm.includes("puma")) return { border: "hover:border-black", bg: "bg-zinc-50/65", badgeBg: "bg-zinc-150 text-zinc-900" };
...
export default function MarketplaceView({
  ...
  return (
    <div className="py-8 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto space-y-8 font-sans">
      ...
      {/* Header Info */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-150/60 pb-6">
              ...
              <div className="flex flex-wrap gap-1">
                {['Amazon', 'Flipkart', 'Swiggy', 'Zomato', 'Myntra'].map(brand => (
                  <button
                    ...
                    className="bg-zinc-50 hover:bg-zinc-100 border border-zinc-150 text-slate-605 text-[9px] px-2 py-0.5 rounded font-bold transition-all cursor-pointer"
                  >
                    {brand}
                  </button>
                ))}
        ...
        <div className="lg:col-span-3">
          {filteredCoupons.length === 0 ? (
            <div className="text-center py-16 bg-white rounded-3xl border border-zinc-200/60 max-w-sm mx-auto">
              <AlertCircle className="w-10 h-10 text-zinc-350 mx-auto mb-3" />
          ...
          {filteredCoupons.map((coupon) => {
            ...
            return (
              <div 
                key={coupon.id}
                className={`bg-white rounded-3xl border border-zinc-200/90 transition-all duration-350 flex flex-col justify-between overflow-hidden relative shadow-sm hover:shadow-xl hover:-translate-y-1 ${theme.border}`}
              >
                ...
                <div className={`h-2.5 w-full bg-gradient-to-r ${
                  ...
                  coupon.brand.toLowerCase().includes('swiggy') ? 'from-orange-400 to-orange-605' :
                  coupon.brand.toLowerCase().includes('zomato') ? 'from-red-500 to-red-650' :
                  coupon.brand.toLowerCase().includes('ajio') ? 'from-zinc-900 to-rose-900' :
                  coupon.brand.toLowerCase().includes('myntra') ? 'from-pink-500 via-purple-500 to-amber-500' :
                  'from-[#ff116d] to-indigo-650'
                }`} />
                ...
                <div className="px-5 pt-3 pb-2 space-y-2 bg-[#fcfdfe]/60 text-xs text-slate-600">
                  ...
                  <div className="flex justify-between items-center text-[10.5px]">
                    ...
                    <span className="font-extrabold text-slate-700 flex items-center gap-1">
                      <span className={`w-1.5 h-1.5 rounded-full ${coupon.ocrExtracted ? 'bg-emerald-500' : 'bg-indigo-550 bg-indigo-500'}`} />
                ...
                <div className="p-5 space-y-3 bg-[#fbfcfd]/20">
                  ...
                  {/* Cost Summary block */}
                  <div className="flex justify-between items-center bg-zinc-50 border border-zinc-150 p-3 rounded-2xl text-xs">
                    <div className="flex flex-col">
                  ...
                  {coupon.status === 'sold' ? (
                    <div className="p-3 bg-indigo-50 border border-indigo-150 rounded-2xl flex flex-col gap-2">
                    ...
                    <div className="space-y-2">
                      <div className="p-3 bg-slate-50 border border-slate-200 rounded-2xl flex flex-col gap-1.5 text-center">
                        ...
                        <span className="font-mono text-slate-650 font-extrabold text-[11px] bg-slate-100 rounded-lg px-2 py-1 truncate">{coupon.code}</span>
                      </div>
                      <button 
                        disabled
                    ...
                    <div className="space-y-2">
                      <button 
                        ...
                        className="w-full bg-indigo-600 hover:bg-indigo-705 active:bg-indigo-800 text-white font-black text-xs py-3.5 rounded-2xl transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50"
                      >
                        {buyingId === coupon.id ? (
      ...
      {quickLookCoupon && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-xs">
          <div className="bg-white rounded-3xl max-w-md w-full overflow-hidden shadow-2xl border border-zinc-150 relative flex flex-col justify-between">
            
            {/* Modal Header */}
            <div className="p-6 bg-zinc-900 text-zinc-100 flex justify-between items-start">
              <div>
                <span className="text-[10px] font-bold uppercase tracking-widest bg-zinc-800 border border-zinc-750 px-2 py-0.5 rounded text-zinc-350">
            ...
            <div className="p-6 space-y-5 overflow-y-auto max-h-[50vh] scrollbar-none text-xs">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-xl border border-zinc-150 text-center space-y-1">
              ...
              <div className="space-y-2 border-t border-zinc-100 pt-4 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px]">
                  <div className="flex justify-between items-center bg-zinc-50 p-2 rounded-lg border border-zinc-150">
            ...
            {/* Modal Actions */}
            <div className="p-6 bg-slate-50 border-t border-zinc-150 flex flex-col gap-3">
              <button
                onClick={() => {
